...
/**
 * 地方经济四期-内蒙古自治区各区县预决算报告-鄂尔多斯-康巴什区-预算
 *


 * 起始请求链接: https://czt.nmg.gov.cn/yjs/business/basic/select?id=150603zf&type=2&p=1
 * 起始请求方式: GET
 */
class EEDS_KBSQ_YS implements IparseScript {
    ...
    ParsePageResult doParse(ParsePageRequest request) {
        ...
        if (parser == null) {
            ...
            for (Map<String, Object> node : nodes) {
                ...
                pageParams.put("webUrl", "http://czt.nmg.gov.cn/yjs/business/page/list?type=2&basicId=150603zf")
        ...
        if (parser == "详情页") {
            ...
            String pageTitle = "地方经济四期\\内蒙古自治区各区县预决算报告\\鄂尔多斯\\康巴什区\\预算"
        ...
        if (parser == "文件详情页") {
            ...
            for (Map<String, Object> node : nodes) {
                ...
                addDataMap(pageResult, dataMap, Pair.of("statistic_area", "统计区域"), "内蒙古自治区鄂尔多斯市康巴什区", true)
    ...
    private static Integer extractReportYear(String title, String releaseDate) {
        String year = ParseToolsUtils.extractFirstGroup(cleanText(title), '(20\\d{2})(?=\\s*(年|年度)?\\s*预算)')
