...
/**
 * 爬虫名称：中国人民银行-调查统计司-统计数据-金融机构信贷收支统计-金融机构人民币信贷收支表
 * 结构：五层 — 百度入口 → 年份列表 → 数据列表 → 详情页 → 下载页
 * 起始URL：http://www.baidu.com
 */
class RmyhXdszJyhq implements IparseScript {
    ...
    ParsePageResult doParse(ParsePageRequest request) {
        ...
        if (parser == "年份列表") {
            ...
            for (Selectable node : nodes) {
                ...
                nextReq.putExtra(CrawlerDataCommonFieldsEnum.SUBTITLE.getName(), yearTitle)
        ...
        // ==========================================
        if (parser == "数据列表") {
            String yearTitle = page.request.getExtra(CrawlerDataCommonFieldsEnum.SUBTITLE.getName())

            List<Selectable> nodes = page.html.xpath("//div[@class=\"portlet\"]/div[2]/table[11]//tr/td").nodes()
            if (nodes.isEmpty()) {
                return pageResult
            }
        ...
        if (parser == "详情页") {
            ...
            // 提取文件下载链接
            String fileHref = page.html.xpath("//table[@class=\"border_nr\"]//table[5]//td[3]/a/@href").get()
            ...
            downReq.putExtra(CrawlerDataCommonFieldsEnum.SUBTITLE.getName(), yearTitle)
            if (StringUtils.isNotBlank(releaseDate)) {
                downReq.putExtra(CrawlerDataCommonFieldsEnum.RELEASE_DATE.getName(), releaseDate)
            }
        ...
        if (parser == "下载页") {
            ...
            String yearTitle = page.request.getExtra(CrawlerDataCommonFieldsEnum.SUBTITLE.getName())
            String releaseDate = page.request.getExtra(CrawlerDataCommonFieldsEnum.RELEASE_DATE.getName())
...
//            String content = page.html.get()
//            content = ParseToolsUtils.buqiHtml(content)
...
//            dataMap.put(CrawlerDataCommonFieldsEnum.CONTENT.getName(),
//                    Pair.of(CrawlerDataCommonFieldsEnum.CONTENT.getAlias(), content))
